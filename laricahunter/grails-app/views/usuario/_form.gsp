...
<div class="fieldcontain ${hasErrors(bean: usuarioInstance, field: 'estabelecimento', 'error')} required">
	<label for="estabelecimento">
		<g:message code="usuario.estabelecimento.label" default="Estabelecimento" />
		<span class="required-indicator">*</span>
	</label>
	<g:select id="estabelecimento" name="estabelecimento.id" from="${laricahunter.Estabelecimento.list()}" optionKey="id" required="" value="${usuarioInstance?.estabelecimento?.id}" class="many-to-one"/>
